...
EXPOSE 8080
CMD ["sh", "-c", "set -eu; mkdir -p /out; printf '%s' \"$YITAPTAP_KEYSTORE_B64\" | base64 -d > /tmp/YiTapTap-release.jks; zipalign -f -p 4 /project/app/build/outputs/apk/release/app-release-unsigned.apk /tmp/YiTapTap-aligned.apk; apksigner sign --v2-signing-enabled true --v3-signing-enabled true --ks /tmp/YiTapTap-release.jks --ks-key-alias yitaptap --ks-pass \"pass:$YITAPTAP_KEYSTORE_PASSWORD\" --key-pass \"pass:$YITAPTAP_KEY_PASSWORD\" --out /out/YiTapTap.apk /tmp/YiTapTap-aligned.apk; zipalign -c -p 4 /out/YiTapTap.apk; apksigner verify --verbose --print-certs /out/YiTapTap.apk; unzip -t /out/YiTapTap.apk >/dev/null; aapt2 dump badging /out/YiTapTap.apk | head -n 8; sha256sum /out/YiTapTap.apk; i=0; base64 -w0 /out/YiTapTap.apk | fold -w 8000 | while IFS= read -r part; do printf 'YITAPTAP_PART_%03d:%s\n' \"$i\" \"$part\"; i=$((i+1)); sleep 0.15; done; echo YITAPTAP_PARTS_END; python3 -m http.server ${PORT:-8080} --directory /out"]
